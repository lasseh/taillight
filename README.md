...
- **Three log feeds** — netlog (network device syslog), srvlog (server syslog), and applog (HTTP ingest) each have independent streams, filtering, retention, and dashboards. Enable only the feeds you need.
- **Three clients** — browser SPA, terminal UI (`taillight-tui`), and SSH-served TUI (`taillight-wish`) — all consume the same SSE streams.
...
- `taillight-tui` — standalone terminal client built on Charmbracelet's bubbletea v2 stack
- Four tabs: dashboard, netlog, srvlog, applog — each with live SSE streaming and a 10k event ring buffer
- Vim-style navigation (`j`/`k`, `g`/`G`, `ctrl+d`/`ctrl+u`), `/` search, `?` help overlay, detail sidebar
- Toast notifications for critical severity (≤3) events within the last 30s
- Tokyo Night theme with TrueColor detection, YAML config at `~/.config/taillight/tui.yml`

### SSH Access

- `taillight-wish` — hosts the TUI over SSH via charm's wish framework; each session spawns its own program instance
- Public-key auth only (`authorized_keys`), with a loud `--allow-any-key` demo mode for public servers
- Per-session cleanup releases SSE goroutines and API connections on disconnect
- Ships session logs (start/end, user, remote addr, key fingerprint, duration) to the applog ingest endpoint
...
`taillight-tui` is a terminal client that connects to any running Taillight API over HTTP/SSE. Four tabs (dashboard, netlog, srvlog, applog), vim navigation, live streaming, search and detail sidebar.

```sh
cd api && make build-tui
./taillight-tui -s https://taillight.example.com -k tl_xxxxx
```

Config file (optional): `~/.config/taillight/tui.yml`
...
CLI flags (`-s`, `-k`, `-c`) override config values.

## SSH Server

`taillight-wish` serves the TUI over SSH. Each session spawns an isolated bubbletea program connected to the Taillight API. Public-key auth is required by default.

```sh
cd api && make build-wish

# Production: public-key auth from an authorized_keys file
./taillight-wish \
...
# Public demo (INSECURE — accepts any key)
./taillight-wish -s https://demo.taillight.example.com --allow-any-key
```

Connect with `ssh -t -p 2222 user@host`. A PTY is required — non-PTY sessions are rejected.

| Flag | Default | Description |
|------|---------|-------------|
| `--listen` | `:2222` | SSH listen address |
| `--host-key` | `.ssh/id_ed25519` | SSH host key path |
| `--authorized-keys` | `~/.ssh/taillight_authorized_keys` | Authorized public keys file |
| `--allow-any-key` | `false` | **Demo only** — accept any public key |
| `-s`, `--server` | *(required)* | Taillight API URL |
| `-k`, `--key` | `""` | API key |
| `--fps` | `30` | Render FPS per session (1–60) |
...
| `api/cmd/taillight-tui/` | Terminal UI client (bubbletea v2) |
| `api/cmd/taillight-wish/` | SSH server that hosts the TUI (charm/wish) |
| `api/internal/tui/` | Shared TUI package: root app, views, client, components, theme |
...
- [Notifications](docs/NOTIFICATIONS.md) — rules, channels, silence/digest model, tuning, troubleshooting
