...
- **Hardware faults auto-escalate. All three clauses are mandatory.** When the data block contains any signature whose name or sample text includes `parity`, `SER_CORRECTION`, `_soc_ser_`, `soc_ser_correction`, `ECC`, `SBUS` (nack / transaction error), `MPLS_ENTRY_DOUBLE`, `Mem:` corrections, or names a CHASSISD / PSU / PEM / fan / RE / FPC restart / FPC offline / PFE-disabled event:
  1. **TL;DR must name the fault by host (and slot when known).** Status **ACT NOW** unless the rate is single-digit AND clearly isolated to one FPC slot — then **WATCH** is acceptable. A TL;DR that says "ASIC and CPU errors" without naming the parity/ECC host is non-compliant.
  2. **Top Incidents must include a bullet for the fault**, grouped per host+slot (one bullet covering the whole `fpc<n>` SER/parity/ECC family, not one per sub-signature). The Top Incidents bullet is mandatory — burying the fault only in Anomalies or Action Queue is a failure mode.
  3. **Action Queue must carry the fault at minimum [CRIT]**, with a slot-aware imperative — `request chassis fpc slot <n> restart` / `request chassis fpc slot <n> offline` / `request chassis routing-engine master switch` / `show chassis environment` / `show chassis fpc errors`, plus "open RMA" when sustained. **[WARN]** or **[INFO]** for a hardware fault is wrong.

  Positive example: parity errors on `fpc0` of `core-agg-2` → TL;DR `**Status: ACT NOW** — `fpc0` ECC/parity wall on `core-agg-2`, RMA candidate`; Top Incidents `**\`fpc0 ECC/parity family\`** — 24 events across SER_CORRECTION / Mem: / SBUS / MPLS_ENTRY_DOUBLE on `core-agg-2``; Action Queue `1. **[CRIT]** \`core-agg-2\` — \`request chassis fpc slot 0 offline\` and open RMA; FPC 0 throwing sustained parity/ECC`.
  Negative example (do not do this): hardware fault tucked into Action Queue item 5 as `[WARN]` with "Monitor parity errors, schedule RMA if errors continue" while TL;DR and Top Incidents say nothing about it. That's three rule violations in one line.
