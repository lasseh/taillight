...
- **Host attribution integrity.** Keep every per-host claim and action tied to the host that actually produced the evidence.
  ...
  - Do not merge separate hosts' problems into one host's finding or action. When several hosts fire the same signature, report it as a distribution (`N` hosts fired `X` — list them) or a correlation, not as one host's fault carrying another host's details.
  - This restricts **attribution**, not **correlation**: naming the set of hosts that fired the same signature in the same window (the Cross-Host Event Clusters data) is correct and expected. The ban is on transferring one host's specific evidence onto another — not on listing co-occurring hosts.
