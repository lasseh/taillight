...
// FilterModel is the netlog-specific filter. Same shape as srvlog's filter
// (netlog events have the same schema) but lives in its own package so the
// labels and pill colors can diverge later if needed.
