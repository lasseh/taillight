...
<template>
  <div class="bg-t-bg-dark border-t-border rounded border p-4">
    <div class="mb-3 flex items-center justify-between">
      <h3 class="text-t-teal text-xs font-semibold uppercase tracking-wide">
    ...
    <div class="space-y-2">
      <div v-for="item in items" :key="item.severity" class="group flex items-center gap-2">
        <span
          class="w-16 shrink-0 text-xs uppercase"
          :class="severityColorClassByLabel[item.label] ?? 'text-t-fg'"
          >{{ item.label }}</span
        >
