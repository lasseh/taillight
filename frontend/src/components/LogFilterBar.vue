...
// Shared srvlog/netlog filter bar. The two feeds are identical except for the
// stores and route/path identifiers, which arrive as props from the thin
// per-feed wrappers (FilterBar, NetlogFilterBar).
interface SyslogFilterStore {
  filters: Record<
    'from' | 'to' | 'hostname' | 'programname' | 'facility' | 'severity_max' | 'search',
    string
  >
...
const props = defineProps<{
  filterStore: SyslogFilterStore
  meta: { hosts: string[]; programs: string[]; facilities: number[] }
  deviceRoute: string
  exportPath: string
  searchLabel: string
}>()
...
function exportCSV() {
  ...
  window.open(`${config.apiUrl}${props.exportPath}?${params}`, '_blank')
}
</script>
...
<template>
  ...
  >
    ...
    <div class="flex items-center gap-3">
      ...
      <RouterLink
        ...
        :to="{ name: props.deviceRoute, params: { hostname: filterStore.filters.hostname } }"
  ...
  <Transition name="filter-panel">
    ...
    >
      ...
      />
      <FilterSelect
        v-model="filterStore.filters.severity_max"
        label="severity"
        :options="severityOptions"
      />
      <FilterSelect
        v-model="filterStore.filters.hostname"
        label="host"
        :options="hostOptions"
        searchable
      />
      <FilterSelect
        v-model="filterStore.filters.programname"
        label="program"
        :options="programOptions"
      />
      <FilterSelect
        v-model="filterStore.filters.facility"
        label="facility"
