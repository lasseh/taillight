...
function switchTab(tab: Tab) {
  ...
  if (
    tab === 'netlog' &&
    (netlogVolume.buckets?.length === 0 || netlogVolume.range !== selectedRange.value)
  ) {
    syncStore('netlog')
  } else if (
    tab === 'srvlog' &&
    (srvlogVolume.buckets?.length === 0 || srvlogVolume.range !== selectedRange.value)
  ) {
    syncStore('srvlog')
  } else if (
    tab === 'applog' &&
    (applogVolume.buckets?.length === 0 || applogVolume.range !== selectedRange.value)
  ) {
    syncStore('applog')
  } else if (
    tab === 'rsyslog' &&
    (!rsyslogStats.summary || rsyslogStats.range !== selectedRange.value)
  ) {
    if (rsyslogPresets.some((p) => p.range === selectedRange.value)) syncStore('rsyslog')
    else rsyslogStats.startRefresh()
  } else if (
    tab === 'taillight' &&
    (!taillightMetrics.summary || taillightMetrics.range !== selectedRange.value)
  ) {
...
const tlEventsData = computed<TlDualRecord[]>(() => {
  const m = mergeTwoLines(
    taillightMetrics.eventsBroadcastLine,
    taillightMetrics.applogBroadcastLine,
  )
  return [...m.entries()]
...
const rsyslogInputs = computed(() =>
  (rsyslogStats.summary?.components ?? [])
    ...
    .map((c) => ({
      name: c.name,
      received: c.stats['submitted'] ?? c.stats['msgs.received'] ?? 0,
    })),
)
...
const rsyslogTileLoading = computed(() => activeTab.value === 'rsyslog' && rsyslogStats.loading)
      ...
      <div class="relative">
        <div v-if="rsyslogStats.summary" class="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <div class="bg-t-bg-dark border-t-border rounded border p-3">
            <div class="text-t-fg-dark text-[10px] font-semibold uppercase tracking-wider">
              Ingest Rate
            </div>
            <div class="text-t-fg mt-1 text-xl font-bold">
              {{ formatRate(rsyslogStats.summary.ingest_rate) }}
            </div>
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-3">
            ...
            <div class="text-t-fg mt-1 text-xl font-bold">
              {{ formatRate(rsyslogStats.summary.filter_rate) }}%
            </div>
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-3">
            <template
              v-if="
                rsyslogStats.summary.total_failed > 0 || rsyslogStats.summary.total_suspended > 0
              "
            >
              <div class="text-t-fg-dark text-[10px] font-semibold uppercase tracking-wider">
                Failures
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-3">
            ...
            <div class="text-t-fg mt-1 text-xl font-bold">
              {{ formatCount(rsyslogStats.summary.main_queue_size) }}
            </div>
            <div class="text-t-fg-dark text-xs">
              max {{ formatCount(rsyslogStats.summary.main_queue_max_size) }}
              <template v-if="rsyslogStats.summary.total_discarded > 0">
                &middot;
                <span class="text-t-red"
                  >{{ formatCount(rsyslogStats.summary.total_discarded) }} discarded</span
                >
              </template>
            </div>
          </div>
      ...
      <div class="relative">
        <div v-if="rsyslogStats.summary" class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div class="bg-t-bg-dark border-t-border rounded border">
            <h3
              class="text-t-fg-dark border-t-border border-b px-3 py-2 text-xs font-semibold uppercase tracking-wide"
            >
              Inputs
            </h3>
            <div
              v-for="inp in rsyslogInputs"
              :key="inp.name"
              class="border-t-border flex items-center justify-between border-b px-3 py-1.5 text-xs last:border-b-0 transition-colors hover:bg-white/[0.03]"
            >
              <span class="text-t-fg">{{ inp.name }}</span>
              <span class="text-t-fg-dark font-mono"
                >{{ formatCount(inp.received) }} <span class="opacity-50">received</span></span
              >
            </div>
            <div
              class="border-t-border flex items-center justify-between border-t px-3 py-1.5 text-xs transition-colors hover:bg-white/[0.03]"
            >
              <span class="text-t-fg-dark font-semibold">Filtered</span>
              <span
                class="font-mono"
                :class="rsyslogFiltered > 0 ? 'text-t-yellow' : 'text-t-fg-dark'"
                >{{ formatCount(rsyslogFiltered) }}</span
              >
            </div>
          </div>
            ...
            >
              ...
              <span class="text-t-fg-dark font-mono">
                ...
                <template v-if="out.suspended > 0">
                  &middot;
                  <span class="text-t-yellow">{{ formatCount(out.suspended) }} suspended</span>
                </template>
              </span>
            </div>
          </div>
    ...
    <template v-if="activeTab === 'taillight'">
      ...
      <div v-if="taillightMetrics.summary" class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div class="bg-t-bg-dark border-t-border rounded border p-3">
          ...
          <div class="text-t-fg mt-1 text-xl font-bold">
            {{ taillightMetrics.summary.db_pool_active }} /
            {{ taillightMetrics.summary.db_pool_total }}
          </div>
          <div class="text-t-fg-dark text-xs">
        ...
        <div class="bg-t-bg-dark border-t-border rounded border p-3">
          ...
          <div class="text-t-fg mt-1 text-xl font-bold">
            {{ formatRate(taillightMetrics.summary.events_rate) }}
          </div>
          <div class="text-t-fg-dark text-xs">
            broadcast/min
            <template v-if="taillightMetrics.summary.events_dropped > 0">
              &middot;
              <span class="text-t-red"
                >{{ formatCount(taillightMetrics.summary.events_dropped) }} dropped</span
              >
            </template>
          </div>
        </div>
        <div class="bg-t-bg-dark border-t-border rounded border p-3">
          ...
          <div class="text-t-fg mt-1 text-xl font-bold">
            {{ formatRate(taillightMetrics.summary.ingest_rate) }}
          </div>
          <div class="text-t-fg-dark text-xs">
            applog/min
            <template v-if="taillightMetrics.summary.applog_ingest_errors > 0">
              &middot;
              <span class="text-t-red"
                >{{ formatCount(taillightMetrics.summary.applog_ingest_errors) }} errors</span
              >
            </template>
          </div>
        </div>
      ...
      <div
        v-if="
          taillightMetrics.summary &&
          (taillightMetrics.summary.events_dropped > 0 ||
            taillightMetrics.summary.applog_ingest_errors > 0 ||
            taillightMetrics.summary.listener_reconnects > 0)
        "
        class="bg-t-bg-dark border-t-border rounded border p-3"
      >
        <div class="text-t-fg-dark text-[10px] font-semibold uppercase tracking-wider mb-2">
          Warnings
        </div>
        <div class="flex flex-wrap gap-3 text-xs">
          <span v-if="taillightMetrics.summary.events_dropped > 0" class="text-t-yellow">
            {{ formatCount(taillightMetrics.summary.events_dropped) }} srvlog events dropped (slow
            SSE clients)
          </span>
          <span v-if="taillightMetrics.summary.applog_events_dropped > 0" class="text-t-yellow">
            {{ formatCount(taillightMetrics.summary.applog_events_dropped) }} applog events dropped
            (slow SSE clients)
          </span>
