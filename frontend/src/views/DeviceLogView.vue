...
import {
  severityLabels,
  severityColorClassByLabel,
  severityBgClass,
  severityBgClassByLabel,
} from '@/lib/constants'
...
const chronologicalCriticalLogs = computed(() =>
  [...(summary.value?.critical_logs ?? [])].reverse(),
)
...
const colWidths = computed(() => {
  const events =
    activeTab.value === 'critical' ? chronologicalCriticalLogs.value : chronologicalLogs.value
      ...
      <div class="shrink-0 space-y-4 p-4">
        ...
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-4">
            ...
            <div class="text-t-teal truncate font-mono text-lg font-bold" :title="summary.hostname">
              {{ summary.hostname }}
            </div>
            <div v-if="summary.fromhost_ip" class="text-t-fg-dark font-mono text-xs">
              {{ summary.fromhost_ip }}
            </div>
            <div
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-4">
            ...
            <div class="text-2xl font-bold">
              <RouterLink
                :to="{ name: listRoute, query: { hostname: summary.hostname, severity: '0' } }"
                class="text-sev-emerg hover:underline"
                >{{ formatNumber(emergCount) }}</RouterLink
              >
              <span class="text-t-fg-dark"> / </span>
              <RouterLink
                :to="{ name: listRoute, query: { hostname: summary.hostname, severity: '1' } }"
                class="text-sev-alert hover:underline"
                >{{ formatNumber(alertCount) }}</RouterLink
              >
            </div>
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-4">
            ...
            <div class="text-2xl font-bold">
              <RouterLink
                :to="{ name: listRoute, query: { hostname: summary.hostname, severity: '2' } }"
                class="text-sev-crit hover:underline"
                >{{ formatNumber(critCount) }}</RouterLink
              >
            </div>
            <div v-if="summary.total_count > 0" class="text-t-fg-dark mt-1 text-xs">
          ...
          <div class="bg-t-bg-dark border-t-border rounded border p-4">
            ...
            <div class="text-2xl font-bold">
              <RouterLink
                :to="{ name: listRoute, query: { hostname: summary.hostname, severity: '3' } }"
                class="text-sev-err hover:underline"
                >{{ formatNumber(errCount) }}</RouterLink
              >
            </div>
            <div v-if="summary.total_count > 0" class="text-t-fg-dark mt-1 text-xs">
            ...
            <div class="-mx-4">
              ...
              >
                <div
                  ...
                  :class="severityBgClassByLabel[msg.severity_label] ?? 'bg-sev-info'"
                />
                <div class="min-w-0 flex-1">
                  <div class="text-t-purple text-[10px] leading-tight">
                    {{ formatNumber(msg.count) }}x &middot; {{ msg.severity_label }}
                  </div>
                  <div
                    class="min-w-0 truncate text-xs leading-snug"
                    v-html="highlightMessage(`${highlightPrefix}:${msg.latest_id}`, msg.sample)"
                  />
                </div>
              </RouterLink>
              <!-- Desktop: full row -->
              ...
              >
                ...
                <span
                  ...
                  :class="severityColorClassByLabel[msg.severity_label] ?? 'text-t-fg'"
                  >{{ msg.severity_label }}</span
                >
                <span
      ...
      <div class="relative flex min-h-0 flex-1 flex-col">
        ...
        >
          ...
          <div
            ...
            @scroll="onLogScroll"
          >
            <component :is="row" v-for="event in currentEvents()" :key="event.id" :event="event" />
          </div>
        </div>
